...
pub async fn fetch_weather(client: &Client) -> Result<WeatherData> {
    // 1. Check in-memory cache
    {
        ...
        if let Some((time, data)) = &*cache {
            if let Ok(elapsed) = time.elapsed() {
                if elapsed < Duration::from_secs(60 * 15) {
                    // 15 minutes
    ...
    let temp = format!(
        "{}°C",
        current["temperature_2m"].as_f64().unwrap_or(0.0).round() as i32
    );
    ...
    let pressure = format!(
        "{} hPa",
        current["surface_pressure"].as_f64().unwrap_or(0.0).round() as i32
    );
    ...
    let wind_gust = format!(
        "{} km/h",
        current["wind_gusts_10m"].as_f64().unwrap_or(0.0).round() as i32
    );
    ...
    for i in current_idx..std::cmp::min(current_idx + 24, hourly_times.len()) {
        ...
        let hour_num = parse_time(&serde_json::Value::String(t_str.to_string()))
            .split(':')
            .next()
            .unwrap_or("0")
            .parse::<u32>()
...
fn get_emoji(code: &str) -> String {
    let c = match code {
        ...
        "143" | "248" | "260" => "🌫️",
        "176" | "353" => "🌦️",
        "179" | "182" | "185" | "227" | "311" | "314" | "317" | "320" | "323" | "326" | "350"
        | "362" | "365" | "368" | "374" | "377" => "🌨️",
        "200" | "386" | "389" | "392" => "⛈️",
        "230" | "329" | "332" | "335" | "338" | "371" | "395" => "❄️",
        "263" | "266" | "281" | "284" | "293" | "296" | "299" | "302" | "305" | "308" | "356"
        | "359" => "🌧️",
        _ => "❓",
    };
    c.to_string()
