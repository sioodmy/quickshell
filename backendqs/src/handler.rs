...
pub async fn handle_request(req: DaemonRequest, ctx: AppContext, assigned_search_gen: Option<u64>) {
    match req {
        ...
        api::DaemonRequest::Dictionary { query } => {
            match dictionary::lookup_word(&ctx.client, &query).await {
                Ok(res) => {
                    let _ = ctx
                        .tx
                        .send(api::DaemonEvent::DictionaryResult {
                            status: "ok".into(),
                            error: None,
                            word: Some(res.word),
        ...
        api::DaemonRequest::Calc { query } => {
            ...
            match rink_core::one_line(&mut rink, &query) {
                Ok(res) => {
                    let _ = ctx
                        .tx
                        .send(api::DaemonEvent::CalcResult {
                            status: "ok".into(),
                            error: None,
                            result: Some(res),
                            query: query.clone(),
        ...
        api::DaemonRequest::Lyrics { artist, title } => {
            match lyrics::fetch_lyrics(&ctx.client, &artist, &title).await {
                Ok(l) => {
                    let _ = ctx
                        .tx
                        .send(api::DaemonEvent::LyricsResult {
                            status: "ok".into(),
                            error: None,
                            lyrics: Some(l),
                        })
        ...
        api::DaemonRequest::WeatherRefresh => match weather::fetch_weather(&ctx.client).await {
            Ok(data) => {
                let _ = ctx
                    .tx
                    .send(api::DaemonEvent::WeatherResult {
                        status: "ok".into(),
                        error: None,
                        data: Some(data),
                    })
        ...
        api::DaemonRequest::AgendaRefresh => {
            if let Ok(items) = agenda::parse_directory(&ctx.notes_dir) {
                let _ = ctx
                    .tx
                    .send(api::DaemonEvent::AgendaUpdate { data: items })
                    .await;
            }
        }
        ...
        api::DaemonRequest::MusicLibrary => {
            let res = tokio::task::spawn_blocking(|| music::scan_library())
                .await
                .unwrap();
            match res {
                Ok(lib) => {
                    let _ = ctx
                        .tx
                        .send(api::DaemonEvent::MusicLibraryResult {
                            status: "ok".into(),
                            error: None,
                            library: Some(lib),
                        })
        ...
        api::DaemonRequest::MusicPlayAlbum {
            tracks,
            start_index,
        } => {
        ...
        api::DaemonRequest::FrecencyLoad => {
            ...
            let _ = ctx
                .tx
                .send(api::DaemonEvent::FrecencyUpdate { scores })
                .await;
        }
        ...
        api::DaemonRequest::BookmarkSearch { query } => {
            ...
            let _ = ctx
                .tx
                .send(api::DaemonEvent::BookmarkSearchResult { query, results })
                .await;
        }
        api::DaemonRequest::AppSearch { query } => {
            ...
            let _ = ctx
                .tx
                .send(api::DaemonEvent::AppSearchResult { query, results })
                .await;
        }
        api::DaemonRequest::FilePreview { path } => {
            let result = tokio::task::spawn_blocking(move || filesearch::load_preview(&path))
                .await
                .unwrap();
            let _ = ctx
                .tx
                .send(api::DaemonEvent::FilePreviewResult(result))
                .await;
        }
        ...
        api::DaemonRequest::SysctlList { kind } => {
            ...
            let _ = ctx
                .tx
                .send(api::DaemonEvent::SysctlListResult { kind, devices })
                .await;
        }
        ...
        api::DaemonRequest::CliphistCopy { raw, image_path } => {
            ...
            let _ = ctx
                .tx
                .send(api::DaemonEvent::CliphistActionDone {
                    action: "copy".into(),
                })
                .await;
        }
            ...
            if guard.is_none() {
                match music_remote::start_server().await {
                    Ok((handle, state)) => {
                        let _ = ctx
                            .tx
                            .send(api::DaemonEvent::MusicRemoteStarted {
                                status: "ok".into(),
                                error: None,
                        ...
                        tokio::spawn(async move {
                            ...
                            loop {
                                ...
                                if !notified_connected
                                    && state_clone.client_connected.load(Ordering::Relaxed)
                                {
                                    notified_connected = true;
                                    let _ =
                                        tx_clone.send(api::DaemonEvent::MusicRemoteConnected).await;
                                }
                                ...
                                if !state_clone.is_active.load(Ordering::Relaxed) {
                                    ...
                                    let _ =
                                        tx_clone.send(api::DaemonEvent::MusicRemoteStopped).await;
                                    break;
                                }
                            }
                        });
            ...
            tokio::task::spawn_blocking(move || {
                if let Ok(entries) = std::fs::read_dir("/sys/class/backlight") {
                    for entry in entries.flatten() {
                        ...
                        if let (Ok(max_str), Ok(_)) = (
                            std::fs::read_to_string(&max_path),
                            std::fs::metadata(&brightness_path),
                        ) {
...
async fn handle_cliphist_list(
    ...
    let _ = tx
        .send(api::DaemonEvent::CliphistListResult { items })
        .await;
...
async fn handle_math(
    ...
    match result {
        Ok((content, path)) => {
            ...
            let _ = tx
                .send(api::DaemonEvent::MathResult {
                    status: "ok".into(),
                    error: None,
                    svg_file: path,
                    svg_content,
