...
fn main() {
    ...
    let match_start = content
        ...
        .unwrap();
    let match_end = content[match_start..]
        .find("});\n            }\n        }\n    }\n\n    Ok(())\n}")
        .unwrap()
        + match_start;
    ...
    let helpers = helpers
        .replace(
            "tmpsc::Sender<DaemonEvent>",
            "tokio::sync::mpsc::Sender<crate::api::DaemonEvent>",
        )
        .replace(
            "tmpsc::Sender<api::DaemonEvent>",
            "tokio::sync::mpsc::Sender<crate::api::DaemonEvent>",
        );
